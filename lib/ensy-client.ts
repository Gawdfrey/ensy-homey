...
export type FanMode = (typeof FAN_MODE)[keyof typeof FAN_MODE];

export const PRESET_MODE = {
	HOME: "home",
	AWAY: "away",
	BOOST: "boost",
} as const;

export type PresetMode = (typeof PRESET_MODE)[keyof typeof PRESET_MODE];
...
export class EnsyClient extends EventEmitter {
	...
	private handleMessage(topic: string, value: string): void {
		...
		switch (key) {
			...
			case "party":
				...
				} else if (this.state.presetMode === PRESET_MODE.BOOST) {
					this.state.presetMode = PRESET_MODE.HOME;
				}
				break;
			case "absent":
				if (value === "1") {
					this.state.presetMode = PRESET_MODE.AWAY;
				} else if (this.state.presetMode === PRESET_MODE.AWAY) {
					this.state.presetMode = PRESET_MODE.HOME;
				}
				break;
			case "textr":
	...
	setFanMode(speed: FanMode): void {
		if (this.state.presetMode !== PRESET_MODE.HOME) {
			this.setPresetMode(PRESET_MODE.HOME);
		}
