...
import { EnsyClient, EnsyState, FanMode, PresetMode } from '../../lib/ensy-client.js';
...
export = class EnsyClimateDevice extends Homey.Device {
  ...
  async onInit() {
    ...
    // Initialize client
    this.ensyClient = new EnsyClient(data.mac_address, data.allow_insecure_tls);
    
    // Set up event listeners
    this.ensyClient.on('stateUpdate', (state: EnsyState, previousState: EnsyState) => {
    ...
    this.registerCapabilityListener('thermostat_mode', this.onThermostatModeChanged.bind(this));
    this.registerCapabilityListener('fan_speed', this.onFanSpeedChanged.bind(this));
  ...
  private updateCapabilityValues(state: EnsyState, previousState?: EnsyState) {
    ...
    if (state.presetMode !== undefined) {
      ...
      if (previousMode && previousMode !== state.presetMode) {
        this.homey.flow.getDeviceTriggerCard('preset_mode_changed')
    ...
    if (state.isHeating !== undefined) {
      const previousHeating = this.getCapabilityValue('heating');
      this.setCapabilityValue('heating', state.isHeating);
    ...
    if (state.temperatureExtract !== undefined) {
      this.setCapabilityValue('measure_temperature.extract', state.temperatureExtract);
    }
    if (state.temperatureExhaust !== undefined) {
      this.setCapabilityValue('measure_temperature.exhaust', state.temperatureExhaust);
    }
    if (state.temperatureOutside !== undefined) {
      this.setCapabilityValue('measure_temperature.outside', state.temperatureOutside);
    }
    if (state.temperatureHeater !== undefined) {
      this.setCapabilityValue('measure_temperature.heater', state.temperatureHeater);
    }
  }
  ...
  private async onFanSpeedChanged(value: number) {
    if (this.ensyClient) {
      try {
        ...
        const fanMode = Math.max(1, Math.min(3, Math.round(value * 3))) as FanMode;
